...
variable "custom_tags" {
  type = map(string)
  description = "(Optional) A map of custom tags as key=value pairs. All tags are added to every resource created by the Auto Scaling Group (instances, volumes, network interfaces, and the security group)."
  default = {}
}
